...
describe("Route", function () {
  ...
  it("should have proper prototype", function () {
    const routeInstance = new RouteParser("/foo");
  ...
  describe("basic", function () {
    it("should match /foo with a path of /foo", function () {
      const route = new RouteParser("/foo");
  ...
  describe("splat parameters", function () {
    it("should handle double splat parameters", function () {
      const route = new RouteParser("/*a/foo/*b");
  ...
  describe("mixed", function () {
    it("should handle mixed splat and named parameters", function () {
      const route = new RouteParser("/books/*section/:title");
  ...
  describe("optional", function () {
    it("should allow and match optional routes", function () {
      const route = new RouteParser("/users/:id(/style/:style)");
    ...
    it("allows optional branches that start with a word character", function () {
      const route = new RouteParser("/things/(option/:first)");
    ...
    describe("nested", function () {
      it("allows nested", function () {
        const route = new RouteParser(
          "/users/:id(/style/:style(/more/:param))"
        );
  ...
  describe("reverse", function () {
    ...
    it("reverses routes with simple params", function () {
      const route = new RouteParser("/:foo/:bar");
    ...
    it("reverses routes with optional params that can't fulfill the optional branch", function () {
      const route = new RouteParser("/things/(option/:first(/second/:second))");
    ...
    it("allows reversing falsy valued params", function () {
      const path =
        "/account/json/wall/post/:id/comments/?start=:start&max=:max";
      const vars = {
        id: 50,
        start: 0,
        max: 12,
  ...
  describe("generate", function () {
    ...
    it("should generate a path with splat params", function () {
      const route = new RouteParser("/foo/*bar/baz");
